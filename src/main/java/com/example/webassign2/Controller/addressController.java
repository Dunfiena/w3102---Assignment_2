...
public class addressController implements addressDao {
    ...
    public Address insert(Address address) throws SQLException {
        ...
        try {
            ...
            address = select(address.getStreet_num(), address.getStreet_name(), address.getPostal());
            return address;
    ...
    @Override
    public Address select(int streetNum, String streetName, String postal ) throws SQLException {
        ...
        try{
            conn = getConnection();
            stmt = conn.prepareStatement("select * from address where street_num=? and street_name=? and postal=?");
            stmt.setInt(1, streetNum);
